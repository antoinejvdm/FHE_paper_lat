...
As can be deduced from the formulas above, the communication cost in one-hot encoding will be lower for small databases (small $n$). In Table \ref{ta:wb:query_PIRANA_OH} the number of elements is shown from which on PIRANA has a lower query communication cost (number of ciphertexts that constitute the query) than one-hot encoding. The results are shown for different amount of slots $s$ and a Hamming weight of $k=2$.



\begin{table}[h]
    \label{ta:wb:query_PIRANA_OH}
    \centering
